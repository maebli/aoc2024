...
fn main() {
    ...
    let field = input
        .chars()
        .fold(HashMap::new(), |mut acc: HashMap<(i32, i32), char>, c| {
            match c {
    ...
    let regions = input.chars().fold(
        HashMap::new(),
        |mut acc: HashMap<char, Vec<Vec<(i32, i32)>>>, c| {
            match c {
                c if c.is_alphabetic() => {
    ...
    let regions_merged: HashMap<char, Vec<Vec<(i32, i32)>>> = regions
        .into_iter()
        .map(|(key, regions)| {
...
fn should_merge(region1: &Vec<(i32, i32)>, region2: &Vec<(i32, i32)>) -> bool {
    region1
        .iter()
        .any(|p1| region2.iter().any(|p2| are_neighbors(*p1, *p2)))
}

fn are_neighbors(p1: (i32, i32), p2: (i32, i32)) -> bool {
    POSSIBLE_EDGES
        .iter()
        .any(|(dx, dy)| p1.0 + dx == p2.0 && p1.1 + dy == p2.1)
}
...
fn find_perimeter(region: &Vec<(i32, i32)>) -> i32 {
    region
        ...
        .map(|p| {
            POSSIBLE_EDGES
                .iter()
                .filter(|(dx, dy)| !region.contains(&(p.0 + dx, p.1 + dy)))
                .count() as i32
        })
        .sum()
}
...
fn find_sides(region_id: char, region: &Vec<(i32, i32)>, field: HashMap<(i32, i32), char>) -> i32 {
    ...
    for p in region.iter() {
        let neighbors = neighbor_directions_inc_diag
            .iter()
        ...
        let corner_pattern = pattern.iter().tuple_windows::<(_, _, _)>().step_by(2);
